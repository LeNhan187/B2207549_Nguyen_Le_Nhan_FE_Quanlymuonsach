<template>
  <header class="sticky top-0 z-50 w-full bg-violet-900/80 backdrop-blur-md shadow-lg text-slate-200 border-b border-gray-700 animate-fade-in-down-header">
        ...
        <nav class="hidden md:flex items-center gap-8">
          ...
          <template v-if="user.isLoggedIn">
            <div class="relative" ref="dropdownRef">
              ...
              >
                <div v-if="dropdownOpen" class="absolute right-0 mt-2 bg-violet-900/80 border border-gray-700 shadow-xl rounded-lg py-2 w-56 z-10 text-slate-200 overflow-hidden">
                  <router-link
                    v-if="user.role === 'admin'"
                    to="/admin"
                    @click="dropdownOpen = false"
                    class="flex items-center gap-3 px-4 py-2 font-semibold text-white hover:bg-violet-700 hover:text-white transition-colors duration-200"
                  >
                  ...
                  <router-link to="/borrowed" @click="dropdownOpen = false" class="flex items-center gap-3 px-4 py-2 hover:bg-violet-700 hover:text-white transition-colors duration-200">
                    <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.246 18 16.5 18c-1.746 0-3.332.477-4.5 1.253" /></svg>
                    <span>Sách đã mượn</span>
                  </router-link>
                  
                  <button @click="handleLogout" class="flex items-center gap-3 w-full text-left px-4 py-2 hover:bg-violet-700 text-red-400 hover:text-red-300 transition-colors duration-200">
          ...
          <template v-else>
            <router-link 
              to="/auth/login" 
              class="font-medium bg-violet-900/90 px-5 py-2 rounded-full hover:bg-violet-600 transition-all duration-300 shadow-lg 
              ...
              <span class="absolute inset-0 bg-gradient-to-r from-violet-600 to-violet-900/90 opacity-0 group-hover:opacity-100 transition-opacity duration-300"></span>
            </router-link>
          </template>
        </nav>
